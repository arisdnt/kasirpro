...
interface PesanSearchFiltersProps {
  searchTerm: string;
...
export function PesanSearchFilters({
  searchTerm,
  statusFilter,
  onSearchChange,
        ...
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
          ...
          <div className="flex flex-1 items-center justify-end gap-4">
            <div className="flex items-center gap-3 bg-white/80 backdrop-blur-sm px-3 shadow-sm border border-slate-300 h-9">
              ...
              <div className="flex flex-col items-center justify-center">
                <span className="text-slate-500 text-[9px] font-medium leading-none">Terkirim</span>
                <span className="font-bold text-xs text-emerald-600 leading-none mt-0.5">{stats.terkirim}</span>
              </div>
              <div className="flex flex-col items-center justify-center">
                <span className="text-slate-500 text-[9px] font-medium leading-none">Draft</span>
                <span className="font-bold text-xs text-amber-600 leading-none mt-0.5">{stats.draft}</span>
              </div>
            </div>
            <Button
              onClick={onRefresh}
